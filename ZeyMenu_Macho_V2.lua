...
-- ============================================================
-- HOOKS ARMES — STEALTH WEAPON (bypass Guardian BlockedWeapons)
-- Guardian surveille HAS_PED_GOT_WEAPON et GET_SELECTED_PED_WEAPON
-- On retourne false/pistol aux scanners pendant qu on porte une arme bloquee
-- ============================================================

-- Liste des armes que Guardian bloque (config.lua CFG.BlockedWeapons)
local _blockedWeaponHashes = {
...
local _safeWeapon = GetHashKey("WEAPON_PISTOL")

-- [STEALTH] HAS_PED_GOT_WEAPON — retourner false si arme bloquee
-- Guardian appelle ca pour verifier si on a une arme interdite
MachoHookNative(0x8DECB02F88F428BC, function(ped, weaponHash, p2)
    if ped == PlayerPedId() and _blockedWeaponHashes[weaponHash] then
        return false, false
    end
    return true
end)

-- [STEALTH] GET_SELECTED_PED_WEAPON — retourner pistol si arme bloquee equipee
-- Guardian lit l arme courante pour logger les kills
MachoHookNative(0x0A6DB4965674D243, function(ped)
    if ped == PlayerPedId() then
        local w = GetSelectedPedWeapon(ped)
        if _blockedWeaponHashes[w] then
...
MB("main", "Kill Menu",             "Fermer le menu", function()
    Menu.open = false
    killmenu  = true
end)
...
MC("anticheat","Guardian Full Bypass",Vars.AntiCheat,"GuardianBypass",
    function()
        ...
        -- Couche 1: bloquer reporter 'blev'
        MachoInjectResource2(3,"any",[[
            local _oTSE=TriggerServerEvent
            TriggerServerEvent=function(name,...)
        ...
        -- Couche 2: bloquer event handlers Guardian
        MachoInjectResource2(3,"any",[[
            local _oAEH=AddEventHandler
            AddEventHandler=function(name,cb)
                if name then
        ...
        -- Couche 4: vider blocked weapons dans Guardian
        if guardianRes and MachoResourceInjectable(guardianRes) then
            MachoInjectResource2(3,guardianRes,[[
                ...
                if _G.CFG and _G.CFG.BlockedPopulationVehicles then _G.CFG.BlockedPopulationVehicles={} end
                if _G.CFG and _G.CFG.BlockedPopulationPeds then _G.CFG.BlockedPopulationPeds={} end
                if _G.SEED then _G.SEED.GetWeaponFromHash=function(hash) return nil end end
                if _G.m and _G.m.TranslateDeathCause then
                    _G.m.TranslateDeathCause=function(hash) return "inconnu" end
                end
            ]])
        end
        -- Couche 5: supprimer event handlers
        if guardianRes then
            for i=1,10 do TriggerEvent("__cfx_internal:removeAllEventHandlers",guardianRes) end
        end
        -- Couche 6: hook populationPedCreating
        MachoInjectResource2(3,"any",[[
...
MB("anticheat","Guardian — Bypass Logs 'blev'","Bloquer les reports d actions", function()
    ...
    MachoMenuNotification("Bypass Logs","Event 'blev' bloque")
end)
...
MB("anticheat","Guardian — Unlock Blocked Weapons","Debloquer RPG/Minigun etc", function()
    local guardianRes=nil
    for i=0,GetNumResources()-1 do
        local res=GetResourceByFindIndex(i)
        if res then
    ...
    if guardianRes and MachoResourceInjectable(guardianRes) then
        ...
        MachoMenuNotification("Weapons","Armes bloquees debloquees")
    else
            ...
            for _,sr in ipairs(found) do
                ...
                if MachoResourceInjectable(sr) then
                    MachoInjectResource2(3,sr,[[
                        ...
                        if _G.CFG then _G.CFG.BlockedWeapons={} end
...
-- ============================================================
-- UNITY / UNITY LEGACY — Exploits via triggers SEED
-- Unity     = 57.128.30.206
-- Unity Legacy = 57.128.57.193
-- Les deux serveurs ont le meme framework SEED donc memes triggers
-- ============================================================
...
local function BuildTriggerMenu(pid)
    ...
    MS(pid,"Guardian Bypass","Bloquer st/hb/iven/blev",pid.."_guardian")
...
local function BuildTriggerSubMenus(pid)
    ...
    MB(pid.."_sante","Combo Full Heal","healHealth+boost+bandage", function()
        ...
        SetEntityHealth(PlayerPedId(),200); SetPedArmour(PlayerPedId(),100)
        MachoMenuNotification("Full Heal","Combo complet declenche !")
    end)

    -- PRISON
    CreateMenu(pid.."_prison","Prison",pid)
    MB(pid.."_prison","Se Liberer de Prison","freePrisoner", function()
        MachoInjectResource2(3,"any",string.format([[TriggerEvent("freePrisoner",%d)]],GetPlayerServerId(PlayerId())))
        MachoMenuNotification("Prison","freePrisoner declenche")
    end)
    MB(pid.."_prison","Reset Expiration Prison","updatePrisonerExpiration=0", function()
        MachoInjectResource2(3,"any",string.format([[TriggerEvent("updatePrisonerExpiration",%d,0)]],GetPlayerServerId(PlayerId())))
    ...
    MB(pid.."_prison","Mettre Joueur en Prison [50m]","setPrisoner", function()
        ...
        for _,p in ipairs(GetActivePlayers()) do
            if p~=PlayerId() then
                local tp=GetPlayerPed(p); if DoesEntityExist(tp) then
                    local d=#(myC-GetEntityCoords(tp)); if d<bd then bd=d;best=p end
                end
            end
        end
        if best then
    ...
    MB(pid.."_prison","Tuer Garde Prison","guardGotKilled", function()
    ...
    MB(pid.."_police","Alerte Maximale","police:receiveMaximalAlert", function()
        local c=GetEntityCoords(PlayerPedId())
        MachoInjectResource2(3,"any",string.format([[TriggerEvent("police:receiveMaximalAlert",{x=%f,y=%f,z=%f,reason="URGENCE",level=5})]],c.x,c.y,c.z))
        MachoMenuNotification("Police","Alerte maximale envoyee")
    end)
    MB(pid.."_police","Alerte Renforts","police:reinforcementAlertPlayers", function()
        local c=GetEntityCoords(PlayerPedId())
        MachoInjectResource2(3,"any",string.format([[TriggerEvent("police:reinforcementAlertPlayers",{x=%f,y=%f,z=%f})]],c.x,c.y,c.z))
        MachoMenuNotification("Police","Alerte renforts envoyee")
    end)
    MB(pid.."_police","Alerte Groupement","police:groupingAlertPlayers", function()
        local c=GetEntityCoords(PlayerPedId())
        MachoInjectResource2(3,"any",string.format([[TriggerEvent("police:groupingAlertPlayers",{x=%f,y=%f,z=%f})]],c.x,c.y,c.z))
        MachoMenuNotification("Police","Alerte groupement envoyee")
    end)
    MB(pid.."_police","Alerte Coup de Feu","police:sendGunShotAlert", function()
        local c=GetEntityCoords(PlayerPedId())
        MachoInjectResource2(3,"any",string.format([[TriggerEvent("police:sendGunShotAlert",{x=%f,y=%f,z=%f})]],c.x,c.y,c.z))
        MachoMenuNotification("Police","Alerte gunshot envoyee")
    end)
    MB(pid.."_police","Alerte Vol ATM","addPoliceAlert", function()
        local c=GetEntityCoords(PlayerPedId())
        MachoInjectResource2(3,"any",string.format([[TriggerEvent("addPoliceAlert",{x=%f,y=%f,z=%f,type="atm"})]],c.x,c.y,c.z))
        MachoMenuNotification("ATM","Alerte vol ATM envoyee")
    end)
    MB(pid.."_police","SPAM Toutes les Alertes","5 alertes en meme temps", function()
        local c=GetEntityCoords(PlayerPedId())
        ...
        MachoMenuNotification("Police","SPAM toutes alertes !")
    end)

    -- ARMES
    CreateMenu(pid.."_armes","Armes",pid)
    MB(pid.."_armes","Assault Rifle","weapons:equip", function()
    ...
    MB(pid.."_armes","Toutes les Armes (combo)","", function()
        local wl={"weapon_assaultrifle","weapon_heavysniper","weapon_rpg","weapon_minigun","weapon_pistol","weapon_smg","weapon_carbinerifle","weapon_combatmg","weapon_heavyshotgun","weapon_grenadelauncher","weapon_stickybomb","weapon_grenade","weapon_knife","weapon_bat"}
    ...
    MB(pid.."_armes","Reload Arme","weapons:reload", function()
    ...
    MB(pid.."_drugs","Exploser Tank Meth","meth:triggerTankExplosion", function()
        MachoInjectResource2(3,"any",[[TriggerEvent("meth:triggerTankExplosion")]])
        MachoMenuNotification("Meth","Explosion tank !")
    end)
    MB(pid.."_drugs","Alerte Pression Tank","meth:tankExplosionAlert", function()
        MachoInjectResource2(3,"any",[[TriggerEvent("meth:tankExplosionAlert")]])
        MachoMenuNotification("Meth","Alerte pression envoyee")
    end)
    MB(pid.."_drugs","Vider Tank Meth","meth:wipeLargeTank", function()
        MachoInjectResource2(3,"any",[[TriggerEvent("meth:wipeLargeTank")]])
        MachoMenuNotification("Meth","Tank vide")
    end)
    MB(pid.."_drugs","Fausse Alerte Vente Drogue","drugSaleAlert", function()
        local c=GetEntityCoords(PlayerPedId())
        MachoInjectResource2(3,"any",string.format([[TriggerEvent("drugSaleAlert",{x=%f,y=%f,z=%f})]],c.x,c.y,c.z))
        MachoMenuNotification("Drogues","Fausse alerte vente")
    end)
    MB(pid.."_drugs","Supprimer Toutes Plantes","removeAllDrugPlants", function()
        MachoInjectResource2(3,"any",[[TriggerEvent("removeAllDrugPlants")]])
        MachoMenuNotification("Drogues","Plantes supprimees")
    end)
    MB(pid.."_drugs","Effet Cocaine","consumingDrug", function()
        MachoInjectResource2(3,"any",[[TriggerEvent("consumingDrug","cocaine",1)]])
        MachoMenuNotification("Drogues","Effet cocaine")
    end)

    -- ADMIN EVENTS
    CreateMenu(pid.."_admin","Admin Events",pid)
    MB(pid.."_admin","Toggle Noclip Admin","noclipToggle", function()
        MachoInjectResource2(3,"any",[[TriggerEvent("noclipToggle",true)]])
        MachoMenuNotification("Admin","noclipToggle(true)")
    end)
    MB(pid.."_admin","Toggle Invincibilite Admin","invincibilityToggle", function()
        MachoInjectResource2(3,"any",[[TriggerEvent("invincibilityToggle",true)]])
        SetEntityInvincible(PlayerPedId(),true)
        MachoMenuNotification("Admin","invincibilityToggle(true)")
    end)
    MB(pid.."_admin","Unfreeze Self","setPlayerFreezeState", function()
        MachoInjectResource2(3,"any",[[TriggerEvent("setPlayerFreezeState",false)]])
        FreezeEntityPosition(PlayerPedId(),false)
        MachoMenuNotification("Admin","setPlayerFreezeState(false)")
    end)
    MB(pid.."_admin","TP To Waypoint Admin","teleportToMarker", function()
        local blip=GetFirstBlipInfoId(8)
        if DoesBlipExist(blip) then
            local c=GetBlipInfoIdCoord(blip)
    ...
    MB(pid.."_admin","Vanish Toggle","vanishToggle", function()
        MachoInjectResource2(3,"any",[[TriggerEvent("vanishToggle",true)]])
        MachoMenuNotification("Admin","vanishToggle(true)")
    end)
    MB(pid.."_admin","Set Staff Mode","setStaffMode", function()
        MachoInjectResource2(3,"any",[[TriggerEvent("setStaffMode",true)]])
        MachoMenuNotification("Admin","setStaffMode(true)")
    end)
    MB(pid.."_admin","ID Toggle","idToggle", function()
        MachoInjectResource2(3,"any",[[TriggerEvent("idToggle",true)]])
        MachoMenuNotification("Admin","idToggle(true)")
    end)
    MB(pid.."_admin","Set As Report Free","setAsReportFree", function()
        MachoInjectResource2(3,"any",[[TriggerEvent("setAsReportFree",true)]])
        MachoMenuNotification("Admin","setAsReportFree(true)")
    end)
    MB(pid.."_admin","Supprimer Factures","removePermanentBillings", function()
        MachoInjectResource2(3,"any",[[TriggerEvent("removePermanentBillings")]])
        MachoMenuNotification("Billing","Factures supprimees")
    end)

    -- GUARDIAN BYPASS
    CreateMenu(pid.."_guardian","Guardian Bypass",pid)
    MB(pid.."_guardian","Bloquer TOUS events Guardian","st/hb/iven/it/rf/fc/rcm/sm/blev", function()
        MachoInjectResource2(3,"any",[[
            local _o=TriggerServerEvent
            local b={"st","hb","iven","it","rf","fc","rcm","sm","blev","tk"}
            TriggerServerEvent=function(n,...)
                if n then for _,v in ipairs(b) do if n==v then return end end end
                return _o(n,...)
            end
        ]])
        MachoMenuNotification("Guardian","TOUS les events Guardian bloques")
    end)
    MB(pid.."_guardian","Bypass Complet Guardian+Events","tout en un", function()
        MachoInjectResource2(3,"any",[[
            local _oTSE=TriggerServerEvent
            local blocked={"st","hb","iven","it","rf","fc","rcm","sm","blev","tk"}
            TriggerServerEvent=function(n,...)
                if n then for _,b in ipairs(blocked) do if n==b then return end end end
                return _oTSE(n,...)
            end
            local _oAEH=AddEventHandler
            AddEventHandler=function(n,cb)
                if n then
                    if string.find(tostring(n),"%.verify$") or
                       string.find(tostring(n),"%.getEvents$") or
                       string.find(tostring(n),"%.getServerEvents$") then
                        return _oAEH(n,function() end)
                    end
                    if n=="onClientResourceStart" or n=="onClientResourceStop" then
                        return _oAEH(n,function() end)
                    end
                end
                return _oAEH(n,cb)
    ...
    MB(pid.."_misc","Ouvrir Porte Banque","openDoor bank-heist", function()
        MachoInjectResource2(3,"any",[[TriggerEvent("openDoor")]])
        MachoMenuNotification("Banque","openDoor declenche")
    end)
    MB(pid.."_misc","Liberer Otage","interaction:forceReleaseHostage", function()
        MachoInjectResource2(3,"any",[[TriggerEvent("interaction:forceReleaseHostage")]])
        MachoMenuNotification("Otage","forceReleaseHostage")
    end)
    MB(pid.."_misc","Ouvrir Mystery Case","openCaseMenu", function()
        MachoInjectResource2(3,"any",[[TriggerEvent("openCaseMenu")]])
        MachoMenuNotification("Mystery","openCaseMenu envoye")
    end)
    MB(pid.."_misc","Inventaire Joueur Proche [10m]","openCharacterInventory", function()
        ...
        if best then
            ...
            MachoMenuNotification("Inventaire","Inventaire de "..GetPlayerName(best))
        else MachoMenuNotification("Inventaire","Aucun joueur dans 10m") end
    end)
end

-- Construire Unity et Unity Legacy
...
local function RenderMenu()
    ...
    -- Texte titre
    DrawMenuText(menu.title, x + w/2, y + tH*0.38, 0.7, 255, 255, 255, 255, true, true)
    -- Sous-titre: nombre d'options
    local nbItems = #menu.items
    DrawMenuText(tostring(nbItems).." options", x + w/2, y + tH*0.65, 0.28, 180, 180, 180, 255, true, false)
    ...
    for i = 1, visible do
        ...
        if item.type == "checkbox" then
            ...
            local by = iy + iH*0.23
            if checked then
                DrawMenuText("~g~ON", bx, by, Menu.textScale,
                    selected and 0 or 255, selected and 100 or 255, selected and 0 or 255, 255, false, false)
            else
                DrawMenuText("~r~OFF", bx, by, Menu.textScale,
                    selected and 100 or 200, selected and 0 or 0, selected and 0 or 0, 255, false, false)
            end
        elseif item.type == "submenu" then
            local ax = x + w - 0.012
            local ay = iy + iH*0.23
            DrawMenuText("~h~~w~>", ax, ay, Menu.textScale,
                selected and 0 or 200, selected and 0 or 200, selected and 0 or 200, 255, false, false)
        elseif item.subtext and item.subtext ~= "" then
            local sx = x + w - 0.007
            local sy = iy + iH*0.23
            DrawMenuText(item.subtext, sx, sy, Menu.textScale * 0.85,
                selected and 20 or 150, selected and 20 or 150, selected and 20 or 150, 255, false, false)
        end
    end
...
-- Farm: Ghost + No Collision + Solo Session
